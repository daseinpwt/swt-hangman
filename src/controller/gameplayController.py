...
class GameplayController:
    ...
    def new_game(self, max_fails, variant):
        ...
        self.run_game(max_fails, variant)

    def run_game(self, max_fails, variant):
        ...
        while not self.has_guessed_word():
            ...
            if self.has_used_letter(letter_input):
                ...
                self.update_number_of_fails(letter_input, max_fails, variant)
        ...
        self.record(1)
        self.request_replay(max_fails, variant)
    ...
    def request_replay(self, max_fails, variant):
        cont = input("do you want to continue? (y/n): ")
        if cont == 'y':
            self.new_game(max_fails, variant)
        elif cont == 'n':
            sys.exit()
            return
        else:
    ...
    def game_over(self, max_fails, variant):
        self.record(0)
    ...
    def update_number_of_fails(self, letter_input, max_fails, variant):
        if not self.word.is_letter_in_word(letter_input):
            self.number_of_fails += 1
        if  self.number_of_fails == self.max_fails:
            self.game_over(max_fails, variant)
